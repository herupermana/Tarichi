...
class The_tag_model extends CI_Model
{
    ...
    public function insert_new($the_post_id = '', $the_tag_name = '')
    {
        $pisah = split(',', $the_tag_name);
        foreach ($pisah as $tag_name) {
            $querycek = $this->db->get_where('the_tag', ['the_tag_name'=>trim($tag_name)]);
    ...
    public function get_per_post($the_post_id)
    {
    ...
    public function delete($the_tag_connector_id)
    {
    ...
    public function get_by_tag($the_tag_name_url, $limit, $start = 0)
    {
    ...
    public function total_per_tag_all($the_tag_name_url)
    {
        $this->db->join('the_post', 'the_post.the_post_id=the_tag_connector.the_post_id');
        $this->db->join('the_tag', 'the_tag.the_tag_name=the_tag_connector.the_tag_name');
        $this->db->where('the_tag.the_tag_name_url', $the_tag_name_url);
